...
static char* do_parse_code(const char* code, const char* filename, unsigned int ast_version, unsigned int flags) {
    ...
    // 3) Conversion en tableau sans dépendre de ast\dump (évite fatal si non dispo)
    // rootArr = { root: <array> , version: {...} }
    zval rootArr;
    array_init(&rootArr);

    zval arr;
    array_init(&arr);
    if (node_to_array(&arr, &ret_node) == SUCCESS) {
        add_assoc_zval(&rootArr, "root", &arr); // ownership transféré
    } else {
        zval_ptr_dtor(&arr);
        add_assoc_string(&rootArr, "error", "node_to_array_failed");
    }
    // cleanup node
    zval_ptr_dtor(&ret_node);

    // 4) Version info
    // version = { php: "...", ast_ext: "...", ast_version: N, engine: "embed" }
    zval ver;
    array_init(&ver);

    // 4.a PHP version (PHP_VERSION)
    {
        zval *c_php = zend_get_constant_str("PHP_VERSION", sizeof("PHP_VERSION") - 1);
        if (c_php && Z_TYPE_P(c_php) == IS_STRING) {
            add_assoc_string(&ver, "php", Z_STRVAL_P(c_php));  // add_assoc_string copie la valeur
        } else {
            add_assoc_string(&ver, "php", "unknown");
        }
    }

    // 4.b ext-ast version : lire la constante AST_VERSION si disponible (évite tout appel userland)
    {
        zval *c_astv = zend_get_constant_str("AST_VERSION", sizeof("AST_VERSION") - 1);
        if (c_astv && Z_TYPE_P(c_astv) == IS_LONG) {
            add_assoc_long(&ver, "ast_ext", Z_LVAL_P(c_astv));
        } else if (c_astv && Z_TYPE_P(c_astv) == IS_STRING) {
            // Certaines versions peuvent exposer une string
            add_assoc_string(&ver, "ast_ext", Z_STRVAL_P(c_astv));
        } else {
            add_assoc_string(&ver, "ast_ext", "unknown");
        }
    }

    // 4.c ast_version utilisé
    add_assoc_long(&ver, "ast_version", (zend_long)version_to_use);
    // 4.d engine
    add_assoc_string(&ver, "engine", "embed");

    add_assoc_zval(&rootArr, "version", &ver); // transfert ownership

    // (Optionnel) inclure le filename hint
    if (filename && *filename) {
        add_assoc_string(&rootArr, "file", (char*)filename);
    }

    // 5) JSON — utilise systématiquement la version userland json_encode pour éviter
    // d'éventuels problèmes d'ABI avec l'encodeur C sur certaines builds.
    char* out = zval_to_json_alloc_via_json_encode(&rootArr);
